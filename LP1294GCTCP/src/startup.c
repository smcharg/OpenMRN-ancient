 ...
 */
static void hard_fault_handler_c(unsigned long *hardfault_arg);
...
static void bus_fault_handler_c(unsigned long *hardfault_arg);

__attribute__((optimize("-O0"))) void bus_fault_handler_c( unsigned long *hardfault_args )
{
	/* force a reference in the local variables for debug */
	volatile FaultInformation *fault_info = &faultInfo;
...
/** The fault handler implementation.  This code is inspired by FreeRTOS.
 */
__attribute__((__naked__)) static void bus_fault_handler(void)
{
	__asm volatile
	(
		" tst   lr, #4               \n"
...
static void bus_fault_handler_x(void)
{
	__asm("BKPT #0\n") ; // Break into the debugger
	for ( ; /* forever */ ; )
	{
